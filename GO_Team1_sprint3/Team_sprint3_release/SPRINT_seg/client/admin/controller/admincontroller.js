     var app = angular.module("myApp", []);
        app.controller("mycntrl", function ($scope, $http) {
            ...
            $scope.arr = [];
            $http.get("http://localhost:5000/initializeProductlist/Productlist").then(function (response) {
            ...
            $scope.Product_itemsfun = function (x) {
                ...
                if ($scope.Product_id.length == 0) {
                    ...
                    //  $scope.start=false;
                    $http.post("http://localhost:5000/insertProduct/insert", $scope.add).then(function (response) {
                        console.log("success");
                        if(response.data){
                        alert("succes");
                        }
            ...
            $scope.remove_items = function (y) {
                ...
                $http.post("http://localhost:5000/deleteProductitem/deleteitem", myOb).then(function (response) {
                    console.log("success");
                    $http.get("http://localhost:5000/initializeProductlist/Productlist").then(function (response) {
                        $scope.finalList = response.data;
                    });
            ...
            $scope.Another_item = function () {
                ...
                for (var i = 0; i < $scope.arr.length; i++) {
                    ...
                    var add = { "pid": $scope.id1, "techspec": $scope.arr[i] };
                   // alert(JSON.stringify(add));
                    $http.post("http://localhost:5000/updateProduct/update", add).then(function (result) {
